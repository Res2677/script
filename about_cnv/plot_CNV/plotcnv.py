...
for i in range(length):
  ...
  a = line.split('\t')
  if (a[0] == chrom):
    if (int(a[1]) >= start and int(a[1]) <= end):
      re.append(i)
...
lll =  args.progress_path + '/' + args.prefix + '.R'
rscript = open(lll,'w')
kk = '''#!/usr/bin/Rscript
library(ggplot2)
...
pdf("'''+ args.progress_path + '''/''' + args.prefix + '''.cnv.pdf",8,8)
...
mtext(side=3,paste("CNV of ''' + args.gene + ''' (","''' + chrom + ''':",''' + str(start)  +''',"-",''' + str(end) +''',")",sep=""),line=1,cex=1.3)
mtext(side=1,"Position in chromosome",line=2,cex=1.3)
dev.off()
'''
rscript.write(kk)
cmd = 'chmod 755 ' + args.progress_path + '/' + args.prefix + '.R'
os.popen(cmd)
time.sleep(1)
cmd =args.progress_path + '/' + args.prefix + '.R'
