...
if (!defined $tsv || !defined $bam || !defined $sample || !defined $out ||defined $help) {
        die << "USAGE";
        description: cluster
...
foreach $fusion (keys %fusion_id)
{
	...
	$cmd = "samtools view $bam|grep -E \'$fusion_id{$fusion}\'|awk \'{print \">\"\$1\" $sample $fusion\\n\"\$10}\'";
	#print "$cmd";
	$fasta = `$cmd`;
	print OUT "$fasta";
	close OUT;
