...
for i in range(fasta_c):
  if (i-1) % 2 == 0:
    ...
    if fu_name in final_dict.keys():
      ...
      #fu_m = fu_name + '\t' + seq
      up_m = up_mess_arr[11]+'-'+up_mess_arr[12]
      down_m = down_mess_arr[11]+'-'+down_mess_arr[12]
      #print >> out_file,fu_m + '\t' + up_m + '\t' + down_m
      addtwodimdict(data,fu_name,'sequence',seq)
      addtwodimdict(data,fu_name,'left part',up_m)
      addtwodimdict(data,fu_name,'left seq',up_seq)
      addtwodimdict(data,fu_name,'left on ref',up_sppos)
      addtwodimdict(data,fu_name,'right part',down_m)
      addtwodimdict(data,fu_name,'right seq',down_seq)
      addtwodimdict(data,fu_name,'right on ref',down_sppos)
